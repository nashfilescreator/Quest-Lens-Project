...
const Oracle: React.FC<AssistantProps> = ({ userId, onClose }) => {
    ...
    const { history, sendMessage } = useOracle(userId);
    ...
    // ... (playAudio and handleVoiceInput remain similar)
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await sendMessage(result.text, 'ai', result.sources);
            if (voiceMode) playAudio(result.text);
        } catch (e) {
            await sendMessage("I'm having a bit of trouble connecting right now. Try again?", 'ai');
        } finally {
            setIsTyping(false);
        }
    };
                ...
                {history.map((msg, i) => {
                    ...
                    return (
                        <div key={i} className={`flex flex-col ${isAI ? 'items-start' : 'items-end'} animate-in slide-in-from-bottom-2 duration-300`}>
                            <div className={`max-w-[85%] px-5 py-3.5 rounded-3xl text-sm leading-relaxed ${isAI
            ...
            <div className="p-4 pb-safe bg-background border-t border-white/50 z-30">
                <form onSubmit={handleSendMessage} className="flex gap-3 items-center">
                    ...
                    <button
                        ...
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg transition-all active:scale-95 ${!inputText.trim() || isTyping
