...
const CreateTeam: React.FC<CreateTeamProps> = ({ onCreate, onCancel, coinCost, userCoins }) => {
    ...
    const generateLogo = () => {
        ...
        setTimeout(() => {
            // Use a static placeholder with dicebear for team avatars
            const url = `https://api.dicebear.com/7.x/shapes/svg?seed=${encodeURIComponent(name)}`;
            setAvatar(url);
                                ...
                                {PRIVACY_OPTIONS.map((opt) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative p-5 rounded-3xl text-left transition-all border ${isSelected
