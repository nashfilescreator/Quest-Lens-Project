      ...
      {availableSkills.map(skill => {
        ...
        return (
          <button
            key={skill.id}
            onClick={handleClick}
            className={`flex flex-col items-center justify-center p-4 rounded-3xl border transition-all relative group h-32 ${isOnCooldown
          ...
          >
            <div className="relative mb-2">
              <CircularProgress
                ...
                strokeWidth={4}
              />
              <div className={`absolute inset-0 flex items-center justify-center transition-colors ${isOnCooldown ? 'text-primary' : skill.color
                }`}>
                {isOnCooldown ? (
