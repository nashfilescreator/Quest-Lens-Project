...
const OptimizedImage: React.FC<OptimizedImageProps> = ({ 
  ...
  const safeSrc = useMemo(() => {
      ...
      try {
          ...
          // Simple check to ensure we aren't passing obviously bad URLs or fixing common encoding issues
          if (src.includes('pollinations.ai') && !src.includes('%20')) {
  ...
  const generatedSrcSet = useMemo(() => {
    if (srcSet || !safeSrc || !safeSrc.includes('pollinations.ai') || !safeSrc.includes('width=')) {
      return srcSet;
    }
    const widths = [400, 800, 1200, 1600];
    return widths.map(w => `${safeSrc.replace(/width=\d+/, `width=${w}`)} ${w}w`).join(', ');
  }, [safeSrc, srcSet]);
